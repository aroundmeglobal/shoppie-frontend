...
export default function ChatPage({ theme = "light", selectedBrand }) {
  useEffect(() => {
    if (selectedBrand) {
  ...
  type Message = {
    sender: string;
    message: string | "";
    user_id: number;  // Correct: Use number instead of Number
  };
  ...
  const handleSend = async () => {
    ...
    addMessage("You", inputValue, anonymousUserId);
    setInputValue("");
  ...
  const handleQuickQuestion = async (question: string): Promise<void> => {
    ...
    addMessage("You", question, anonymousUserId);
